...
loginController.register = (req, res) => {
    ...
    // Usar el nombre como _id (puedes cambiar esto por otro valor personalizado)
    const userData = { _id: nombre, nombre, email, password, rol: rol || 'usuario' };
    loginDAO.findByNombre(nombre)
        .then((existingUser) => {
            ...
            return loginDAO.insert(userData);
        })
        .then((user) => {
            if (user) {
...
loginController.login = (req, res) => {
    ...
    loginDAO.findByNombre(nombre)
        ...
        .then((user) => {
            if (user) {
                res.json({ message: "Inicio de sesión exitoso 😁", user: { id: user._id, nombre: user.nombre, rol: user.rol } });
            }
        })
        .catch((error) => {
            if (!res.headersSent) {
